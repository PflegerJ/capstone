...
def train_model(boss_ID, difficulty, steps):
    df = pd.read_csv(boss_ID + "_" + str(difficulty) + '_' + str(steps) + '.csv')   
    df = df.dropna()
    ...
    for i in range(df.shape[0]):
        ...
        if tag_df.iloc[i] == 1:
            ...
            kill.append(temp_list)
        else:
            wipe.append(temp_list)

    X = np.array(X)

    print(len(kill) / (len(kill) + len(wipe)))
    ...
    # Save model
    model.save("models/" + str(boss_ID) + "_" + str(difficulty) + "_" + str(steps) + ".model")
